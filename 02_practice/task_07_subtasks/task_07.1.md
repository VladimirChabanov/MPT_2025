...
Нам понадобится две чистые виртуальные машины. Если у вас дефицит дискового пространства, то ВМ из предыдущих работ можно удалить, здесь они нам не понадобятся.  
...
![](./task_07.1_img/net.jpeg)

1. Создайте две виртуальные машины с Ubuntu Server (можно использовать существующие). В процессе установки используйте стандартные настройки. Из дополнительного софта понадобится только ssh-сервер.  
   Для машины с именем "Internet" создайте диск большого объёма 50+ГБ.

   > Реальный объём дискового пространства занятого изначально чистыми ВМ в конце работы будет:
   >
   > - "Ansible" ~ 8G;
   > - "Internet" ~ 15G.

2. В настройках сети VirtualBox установите "Сетевой мост" для обеих машин.

#### Машина "Ansible"

Установим Ansible согласно инструкции с [оф сайта](https://docs.ansible.com/ansible/latest/installation_guide/intro_installation.html#installing-and-upgrading-ansible-with-pipx). Ansible написан на Python, поэтому его можно установить как обычный Python-пакет.
В зависимости от версии Python (в новых версиях) во время установки Ansible через pip можно получить ошибку связанную с тем, что pip откажется устанавливать пакет в глобальное окружение ([PEP 668](https://peps.python.org/pep-0668/)), а если установить его в виртуальное окружение, то каждый раз для использования Ansible придётся это окружение активировать. На оф. сайте рекомендуют устанавливать Ansible через [`pipx`](https://pipx.pypa.io/stable/), который автоматически создаёт изолированные окружения для Python-приложений.
...
3. Обновите индексы пакетов: `sudo apt update`.
...
5. Установите `pipx`:
   ...
   pipx ensurepath  # Добавит /home/<имя_пользователя>/.local/bin в PATH
   ```

6. Перелогиньтесь, чтобы обновилось значение переменной `PATH`.

7. Установите последнюю доступную версию Ansible и пакет для автодополнения по одинарному и двойному нажатию <kbd>Tab</kbd>:
...
8. Перелогиньтесь, чтобы автодополнение начало работать.

9. Теперь убедитесь, что ansible установился: `ansible --version`.  
...
1. Установим docker и docker compose согласно инструкции с [оф сайта](https://docs.docker.com/engine/install/ubuntu/) или при помощи скрипта:
...
2. Добавьте вашего пользователя в группу `docker`.
   ...
   если группы не существует, создайте её

   ```bash
   sudo groupadd docker
   ```

3. Перелогиньтесь, чтобы список групп вашего пользователя обновился.

### Подготовка инфраструктуры

Приложение, которое мы будем разворачивать будет использовать следующую конфигурацию:

![](./task_07.1_img/servers_map.svg)
...
- Сервер с базой данных. По хорошему, нагрузку на базу данных тоже нужно балансировать, но это сильно усложнит задачу, т.к. кроме балансировки, нужно ещё и обеспечивать синхронизацию.

Все сервера будут основаны на Ubuntu и кроме возможности доступа по ssh никакого дополнительного софта на них установлено не будет, по крайней мере в начале. Для работы ansible на управляемых серверах должен быть установлен Python, но он установлен в Ubuntu по умолчанию.
...
#### Готовим образ сервера ubuntu-ssh

Для создания образа сервера воспользуемся докерфайлом из [этого репозитория](https://github.com/rastasheep/ubuntu-sshd).

1. На виртуальной машине с докером создайте каталог `"ubuntu-sshd"` и перейдите в него;

2. Создайте `"Dockerfile"` содержащий:
   ...
   Как видно, для пользователя root устанавливается пароль "root" и в настройках "sshd_config" параметру `PermitRootLogin` устанавливается значение "yes", т.е. мы разрешаем подключение по ssh под пользователем root.

3. Используя докерфайл соберите образ "ubuntu-sshd:22.04".

#### Создаём сервера

Прежде, чем приступить к созданию серверов, нужно собрать некоторые сведения о сети. Чтобы дать контейнерам доступ наружу (к роутеру) так, как будто каждый из них - это реальный компьютер, мы создадим docker-сеть с драйвером `ipvlan`. Для настройки этого драйвера, нам нужно знать имя сетевого интерфейса, через который виртуальная машина подключена к роутеру, адрес подсети, к которой мы хотим подключится и gateway, чтобы иметь возможность выйти из сети наружу (например в интернет).

1. Выполните команду `ip a` и найдите **сетевой интерфейс**, который подключён к вашему роутеру.

   ![](./task_07.1_img/interface.png)
...
2. Выполните команду `ip r` и найдите `default`. Этот ip - **gateway** (в нашем случае это ip роутера).
   ...
   ![](./task_07.1_img/route.png)

3. Теперь нужно посмотреть занятые ip адреса в нашей локальной сети. Это нужно, чтобы выбрать диапазон свободных ip для наших серверов.  
   Установите утилиту `arp` при помощи команды: `sudo apt install net-tools`, а затем выполните `arp -en`.  IP-адреса из списка заняты.

   ![](./task_07.1_img/busy_ip.png)
...
5. На сервере с docker compose создайте файл `"compose.yaml"` и откройте его в текстовом редакторе.

6. Поместите в него следующий текст (с учётом ваших параметров сети):

   ```yaml
   services:
     test: # Так я назвал сервис
       image: ubuntu-sshd:22.04 # Наш базовый образ
       dns:
...
8. Проверьте, что к этой машине можно подключиться как и к любой другой по ssh (ip, port и пользователь вам известны из `"compose.yaml"`).

9. Наберите в терминале:
...
10. Откройте файл `"compose.yaml"` и поместите следующий текст (с учётом ваших параметров сети):
...
11. Запустите машины командой: `docker compose up -d` и на некоторое время мы перейдём на машину с ansible.

### Соединяем Ansible с управляемыми машинами

Одно из преимуществ ansible заключается в том, что на управляемые сервера НЕ нужно устанавливать дополнительный софт (ansible-клиент), достаточно просто иметь к ним доступ.
...
1. На машине с Ansible выполните команду:
...
5. Снова выполните команду:
   ...
   В конфиге мы указали, что файл инвентаря лежит в той же папке, что и конфиг и называется `"hosts"`. Обычно так и поступают, но технически можно было положить инвентарь в другое место.

6. Создайте файл `"hosts"` и напишите в нём следующее (только ip-адреса укажите свои):
...
7. Убедитесь, что ansible подхватил информацию о хостах:
...
8. Попробуем подключится к ним.  
   ...
   В итоге вы должны были получить ошибку, т.к. при подключении к серверам по ssh **по паролю** нужно вводить этот самый пароль, а ansible такое не умеет, но может научиться, если дополнительно установить пакет: `sshpass`. 

9. Установите пакет `sshpass` при помощи `apt` и повторите `ping`.

10. В этот раз текст ошибки должен измениться. Суть ошибки в том, что мы не подтвердили, что доверяем этим серверам (в работе №2 мы отвечали yes при первом подключении). Для решения этой проблемы есть 3 основных способа:

    - Подключится к каждому серверу по отдельности и подтвердить доверие;
    - Добавить публичные ключи серверов в `"known_hosts"` машины с ansible (вручную или через `ssh-keyscan`);
...
11. В этот раз подключение должно быть успешно, и все три сервера должны ответить "pong".
    ...
    Вывод от каждого сервера будет сопровождаться [предупреждением](https://docs.ansible.com/ansible-core/2.11/reference_appendices/interpreter_discovery.html) (warning) о том, что ansible использует Python который был найден на сервере автоматически и в случае установки другого интерпретатора, в будущем, может быть использован уже он (это может привести к потенциальным проблемам). Чтобы убрать предупреждение можно прописать конкретный путь к интерпретатору либо указать молчаливый режим поиска `auto_silent`.

    Чтобы убрать предупреждение на глобальном уровне добавьте в `"ansible.cfg"` строку: `interpreter_python = auto_silent`.
...
5. Проверьте, что **все** машины доступны при помощи модуля ansible `ping`. 

6. Пропингуйте все машины из группы `workers`.

7. Пропингуйте все машины из группы `prod`.
...
Сервера которыми мы будем в дальнейшем управлять через ansible были созданы на основе официального образа "ubuntu:22.04", a там указаны официальные репозитории пакетов, но т.к. мы под санкциями, официальные репозитории пакетов Ubutnu могут быть не доступны (периодически отваливаться или вообще не отвечать), поэтому заменим их на зеркала от Яндекса.

Для этого достаточно в файле `"/etc/apt/sources.list"` заменить строки "archive.ubuntu.com" и "security.ubuntu.com" на "mirror.yandex.ru". Выполним это на всех серверах пи помощи модуля [`shell`](https://docs.ansible.com/ansible/latest/collections/ansible/builtin/shell_module.html), который позволяет выполнить любую команду так, как будто вы её набрали в терминале на управляемой машине:
...
Если всё отработано правильно, то сервер должен ответить `CHANGED`.
...
2. Проверьте, что копирование файла прошло успешно. Для этого пропингуйте все сервера командой:
   ...
   Здесь ключ `-i` позволяет явно указать инвентарь который ansible должен использовать для выполнения команды.
...
3. Обновим индексы apt-пакетов на сервере "worker4", для этого будем использовать модуль ansible [`apt`](https://docs.ansible.com/ansible/latest/collections/ansible/builtin/apt_module.html):
...
7. Теперь настало время создать пользователя под которым в дальнейшем будет работать ansible:
   ...
   > Если появится ошибка "No module named 'passlib'", то установите его:
   >
   > ```bash
...
8. Настроим на машинах из группы "staging" доступ под пользователем "ansible" по ключу.  
   Для этого первым делом нам нужна пара приватный/публичный ключ которую мы будем использовать для доступа.
   ...
   - Добавим на сервера группы "staging" наш публичный ключ в файл `"~/.ssh/authorized_key"` с помощью модуля [`authorized_key`](https://docs.ansible.com/ansible/latest/collections/ansible/posix/authorized_key_module.html). Вернитесь обратно в каталог `"~/app/ansible"` и выполните команду:
...
10. Подождите несколько минут и запустите пинг **всех** серверов (через ansible). Вы должны увидеть, что сервер из группы "staging" ответит ошибкой (`UNREACHABLE`). Если ответит `SUCCESS`, то подождите ещё и пропинугие повторно.
...
На данный момент у нас должна быть следующая структура каталогов:
...
2. Добавьте в файл следующие строки:
   ...
   Здесь мы пишем название сценария `name`, затем указываем на каких хостах он должен выполняться `hosts`.
...
2. Добавьте в файл следующие строки:
   ...
   Здесь появился новый параметр - `become`. Он говорит, что команды нужно выполнять от имени некоторого пользователя (по умолчанию `root`). Воспринимайте его просто как `sudo`. В ad hoc командах аналогом является флаг `-b`, но мы его не использовали, т.к. и так подключались к машинам от рута. В дальнейшем мы будем использовать пользователя "ansible", поэтому этот параметр нужен.
...
4. Запустите palybook `"setup.yaml"` для всех машин и дождитесь его завершения. В логах вы должны увидеть, что на всех серверах кроме "worker4" на каждом этапе происходили изменения (changed), а на "worker4" изменений не было (ok), т.к. мы выполнили указанные действия вручную в предыдущем разделе.

5. Подождите несколько минут и запустите `"ping.yaml"`. Теперь, все сервера, кроме "worker4" перестали отвечать, т.к. в файле `"hosts"` указаны данные для пользователя root.
...
2. Добавьте в файл `"compose.yaml"` дополнительные сервера, таки образом, чтобы *рабочих* серверов (воркеров) было 8;

3. Запустите их при помощи `docker compose up -d`. Все машины будут чистыми с настройками по умолчанию;

4. Добавьте новые сервера в файлы `"hosts_root"` и `"host"` в группу "prod".

5. Пропингуйте сервера при помощи плейбука `"ping.yaml"` указав в качестве инвентаря `"hosts_root"` (т.к. сервера сброшены в начальное состояние).

6. Запустите начальное конфигурирование указав в качестве инвентаря `"hosts_root"`.

   Из-за большого количества машин может быть большая нагрузка на сеть и как следствие на нескольких машинах может появится ошибка вида: "connection timed out". В этом случае запустите playbook ещё раз.

7. Подождите несколько минут и пропингуйте сервера указав в качестве инвентаря `"hosts"`.

8. Запустите установку всего необходимого софта для сервера базы данных, указав в качестве инвентаря `"hosts"`.

9. Запустите установку всего необходимого софта для рабочих серверов, указав в качестве инвентаря `"hosts"`.

10. В файле `"install_haproxy.yaml"`, в конфиге балансировщика, в раздел "backend" добавьте новые рабочие сервера и тоже запустите установку.

11. Протестируйте работоспособность приложения. Можно просто обновить вкладку раз 20-30.

12. Зайдите на страницу "/haproxy_stats" и проверьте, что все сервера работают и загружены равномерно.
...
- Небезопасно хранить такие playbook-и в удалённых системах контроля версий. Даже из приватных репозиториев случаются утечки.
