...
32. Модифицируйте пользовательский конфиг на хостовой машине следующим образом:
    ...
    - Создайте секцию `Host vps`, для подключения к *VPS*. В секции укажите нужные для подключения параметры (доступны [тут](https://moodle.cfuv.ru/pluginfile.php/1564677/mod_resource/content/1/vps.pdf)).
...
34. Для начала выполним базовые настройки безопасности *ssh*-сервера. Для этого отредактируем конфигурационный файл `"/etc/ssh/sshd_config"`. Откройте его в любом текстовом редакторе (через `sudo`):
    ...
    - Довольно часто боты сканируют 22 порт на наличие ответа от *ssh*-сервера и пытаются подобрать пароль к нему по базе самых популярных паролей. Изменим порт для подключения по *ssh* на любой свободный, например на 6543. Это не спасёт нас от взлома, если этого сильно захотят, но зато мы не будем тратить ресурсы нашего сервера на установку "мусорных" подключений со случайными ботами. При необходимости, можно указать несколько портов повторив соответствующий параметр несколько раз (в данном случае будет не переопределение, а добавление).
      ...
      ListenStream=6543</pre>
              Первый, пустой <code>ListenStream</code> нужен, чтобы обнулить настройки по умолчанию. Без этой строки, по ssh можно будет подключиться и по 6543 и по 22 порту.<br>
              Если в секции <code>[Socket]</code> будет строка <code>BindIPv6Only=ipv6-only</code> удалите её, иначе сокет не будет принимать подключения по IPv4.</li>
          </ul>
      </details>
...
108. На хостовой машине откройте и изучите файл `known_hosts`.
     ...
     Как видно, файл содержит все 3 ключа сервера а не один, для каждого из которых указано: *IP* машины, формат ключа и сам публичный ключ. Кроме перечисленных полей могут присутствовать и другие ([подробнее](https://manpages.ubuntu.com/manpages/plucky/en/man8/sshd.8.html#ssh_known_hosts%20file%20format)).
...
109. Откройте файл `known_hosts` в текстовом редакторе и оставьте там только ключ в формате `ssh-ed25519`, остальные два удалите. Сам ключ замените на следующий:

     ```bash
     AAAAC3NzaC1lZDI1NTE5AAAAIExc+1ev4tY8mJe7WwIHMknoCZPxA2Kw9sOIixNGQPeu
     ```
...
114. Перенаправьте вывод команды `ssh-keyscan` так, чтобы **дописать** его в вконец файла `known_hosts`.

115. На виртуальной машине перейдите в каталог `"/etc/ssh/"` и удалите все ключи *ssh*-сервера.

116. В терминале хостовой машины попробуйте подключиться к виртуальный машине.

     Как видно, процесс подключения был прерван, т.к. у сервера нет ключей и теперь он не может перейти на шифрованное соединение.

117. На сервере, в каталоге `"/etc/ssh/"`,  выполните команду:

     ```bash
     sudo ssh-keygen -A
     ```

     В результате будут сгенерированы новые пары ключей сервера (старые ключи заменяться не будут, если они существуют).

118. В терминале хостовой машины повторно попробуйте подключиться к виртуальный машине.

     Теперь вы снова получите ошибку. На этот раз она говорит, что ключи виртуальной машины не совпадают с теми, которые в `known_hosts`. Так и должно быть, мы же их обновили ключи сервера.

119. Воспользуетесь командами `ssh-keygen -R` и `ssh-keyscan`, чтобы удалить старые ключи из `known_hosts` и добавить туда новые.
...
127. Чтобы не заниматься ручным перекладыванием публичного ключа на сервер можно воспользоваться утилитой [`ssh-copy-id`](https://manpages.ubuntu.com/manpages/trusty/man1/ssh-copy-id.1.html). В терминале хостовой машины введите команду:
     ...
     > ❗ Если вы видите сообщение, что команда не найдена, то установите утилиту `git` (https://git-scm.com/) и откройте оболочку, которая идёт в комплекте (ПКМ в папке с клюём и в контекстном меню выбрать "Open Git Bash here"). `git` устанавливает всё необходимое для его работы в собственную папку, в том числе и все утилиты из набора OpenSSH.
...
163. Отправьте публичный ключ `"server3.pub"` на виртуальную машину и добавьте в `"authorized_keys"`.
     ...
     **Внимание!** Ключи в файле `"authorized_keys"` должны быть на отдельных строках. Убедитесь, что новый ключ не приклеился в конец последнего, иначе оба ключа будет нерабочие. Как вариант, перед добавлением ключа можно добавить в `"authorized_keys"` символ перевода на новую строку:

     ```bash
     echo '' >> authorized_keys
     ```

     Здесь мы дописываем "ничего" (пустые кавычки) в файл. По умолчанию, `echo` добавляет перевод строки в конец автоматически (такое поведение можно отключить опцией `-n`).
