# Практическое задание 4. Знакомство с Docker Swarm
...
6. Чтобы покинуть кластер и выйти из режима Swarm введите:
   ...
   Узлы диспетчера Swarm используют [Алгоритм консенсуса](https://digitology.tech/docs/docker/engine/swarm/raft.html#engine-swarm-raft) для управления состоянием кластера. Т.е. оставшиеся менеджеры должны образовывать большинство (кворум), чтобы кластер мог корректно обновлять своё состояние иначе он перестанет принимать новые команды.
...
39. Создайте файл "v2.yaml" содержащий:
    ...
    Этот файл дополнен новой секцией `deploy`, которая определяет, что сервис должен запустить 2 контейнера (`replicas`) и при этом каждый контейнер будет ограничен половиной процессорной мощности машины и 100 мегабайтами оперативной памяти;
...
![](./task_04_img/congif_secrets.png)
...
81. Откройте файл "stack.yaml" и замените его содержимое на:
    ...
    В данной версии приложения изменены версии контейнеров на "v3", а также сервису "numbers-api" был добавлен блок:
...
90. Удалите все стеки и сервисы из кластера.

<br>

## Кластер из нескольких машин

Перед выполнением этого пункт создайте 2 клона своей витальной машины (в отдельные каталоги) или создайте 2 новых ВМ и установите на них Docker. В процессе клонирования будет создана точная копия исходной ВМ, поэтому, чтобы не было проблем с сетью, в окне "Клонировать виртуальную машину", в выпадающем списке "Политика MAC-адреса" выберите "Сгенерировать новые MAC-адреса всех сетевых адаптеров". Сеть на ВМ должна работать в режиме "Сетевой мост".

После выполнения этого раздела, клоны будут больше не нужны.

Если у вас техника не может запустить одновременно 3 виртуальные машины, то в качестве альтернативы, можете выполнить эти пункты в группе, например на паре (нужно быть в одной сети и сеть на ВМ должна работать в режиме "Сетевой мост").

91. Запустите 3 виртуальные машины и при помощи `ping` проверьте, что они "видят" друг друга.

92. Выберите любую машину на которой у вас есть файл "v2.yaml" созданный ранее и выполните команду:

    ```bash
    sudo hostname node1
    ```

    Эта команда позволит изменить имя машины на `node1` до ближайшей перезагрузки. Если вы хотите изменить имя на постоянной основе воспользуйтесь командой:

    ```bash
    sudo hostnamectl set-hostname node1
    ```

    Чтобы вы увидели изменения, нужно перелогиниться.

93. На второй машине поменяйте имя на `node2`.

94. На первом узле ("node1") уже должен быть инициализирован режим swarm. Чтобы подключить вторую машину, в режиме менеджера, нужно узнать токен. Выполните команду:

    ```bash
    docker swarm join-token manager
    ```

    В результате на экране появится команда, которую нужно выполнить на подключаемой машине для добавления к кластеру. В команде сразу будет встроен токен, IP-текущей машины и порт.

95. Выполните эту команду на `node2`.

    В результате вы должны получить сообщение, о том, что машина подключена к кластеру как менеджер.

96. На любом менеджере введите команду:

    ```bash
    docker node ls
    ```

    В результате, вы получите список узлов подключённых к кластеру. В столбце `MANAGER STATUS` один из узлов отмечен как лидер (Leader), а второй как доступный (Reachable).

97. На узле "node2" выполните команду:

    ```bash
    docker swarm leave --force
    ```

    Теперь узел перестал быть частью кластера и вышел из режима Swarm. Кластер при этом **не получил** никаких уведомлений и продолжает считать "node2" своей частью.

98. На узле "node1" выполните команду:

    ```bash
    docker node ls
    ```

    Вы должны получить ошибку, содержащую текст "..too few managers are online.." (слишком мало менеджеров онлайн).

    Как говорилось ранее, в случае отключения менеджеров от кластера, должен быть избран новый лидер (переизбирается всегда), но для этого нужно, чтобы в сети оставалось $(N/2)+1$ менеджеров, иначе кластер перестаёт принимать новые команды (хотя приложения в кластере продолжают работать). В нашем случае $N=2$, поэтому для переизбрания нового лидера должны быть доступны 2 узла-менеджера, а у нас только один.

99. На узле "node1" выполните команду:

    ```bash
    docker swarm init --force-new-cluster
    ```

    Это заставить текущую машину пересоздать кластер с собой в качестве менеджера. Если в кластере были стеки, они пересоздадутся.

100. На узле "node1" получите токен для подключения воркера:

     ```bash
     docker swarm join-token worker
     ```

101. Подключите узел "node2" к кластеру в качестве воркера.

102. На узле "node1" выполните команду:

     ```bash
     docker node ls
     ```

     Как видно, "node2" в списке присутствует дважды (с разными ID). Один из них отмечен как `Ready`, а второй как `Down`.

     Как упоминалось ранее, при отключении "node2" кластер не получил уведомлений и продолжил считать "node2" своей частью, поэтому, при перезапуске кластер, swarm попытался восстановить подключение ко всем его бывшим членам.

103. Удалите "node2" с отметкой `Down` из кластера командой:

     ```bash
     docker node rm ID # ID "node2" с отметкой Down
     docker node ls    # Проверяем
     ```

104. В файле "v2.yaml" измените количество реплик на 4 и запустите на основе него стек с именем `app`.

105. Проверьте на каких машинах были запущены контейнеры (столбец `NODE`):

     ```bash
     docker stack ps app
     ```

     Должно быть по 2 контейнера на каждой машине кластера, но возможно и другое распределение.

     Если на какой-то машине кластера не будет необходимого образа, она автоматически его скачает с интернета (НЕ с других машин кластера).

106. На узле "node1" выполните команды:
     ...
     Узел "node2" перешёл в режим технического обслуживания (но от кластера не отключился). Контейнеры, которые были запущены на "node2", были бы остановлены и запущены на "node1" (т.к. в кластере больше нет других активных узлов). Режим drain для менеджера означает, что он не запускает контейнеры, но принимает участие в управлении как и другие менеджеры.
     Кроме режима `drain`, аvailability может быть `active` и `pause`.

107. На узле "node1" выполните команды:
     ...
     Как видно, даже после возвращения "node2" контейнеры продолжили работать на "node1".

108. На "node1" выполните команду:

     ```bash
     docker node promote node2
     ```

     В результате "node2" был повышен до менеджера.

109. На третьей машине поменяйте имя на "node3" и добавьте "node3" в кластер как менеджер.

     Теперь у нас в кластере 3 менажёра.

110. Обновите приложение добавив ещё 2 реплики и посмотрите как они распределились по узлам кластера:

     ```bash
     docker service update --replicas 6 app_todo-web
     docker stack ps app
     ```

     Как видно, новые контейнеры были запущены на "node2" и "node3", а старые так и остались на "node1".

111. На "node1" выполните команду:

     ```bash
     docker swarm leave --force
     ```

     Как и раньше мы отключаем менеджера от кластера, но в этот раз у нас формула $(N/2)+1$ даёт 2. Т.е. в случае 3-х менеджеров, отключение одного из них не приведёт к выходу кластера из строя.

112. Проверьте, что "node2" всё ещё реагирует на команды:

     ```bash
     docker node ls
     ```

113. Удалить "node1" из кластера выполнив на "node2" команду:

     ```bash
     docker node rm node1
     ```

     В результате вы должны получить ошибку. Т.к. кластер думает, что узел всё ещё является частью кластера и более того является менеджером (отключение менеджеров не безопасно, как мы видели ранее).

114. На "node2" выполните команду:

     ```bash
     docker node demote node1
     ```

     В результате узел "node1" будет понижен до worker-а.

115. На "node2" повторено попробуйте удалить "node1" из кластера:
     ...
     Теперь удаление узла безопасно и оно должно пройти успешно.

116. На "node2" выполните команду:

     ```bash
     docker stack ps app
     ```

     Как видно, те контейнеры, которые были запущены на "node1" были остановлены (ещё на моменте `docker swarm leave --force`) и запущены на "node2" и "node3".

117. На "node2" выполните команды:
     ...
     Как видно, после того, как узел понизил себя до worker-а, он может безопасно отключиться от кластера, поэтому флаг `--force` не нужен.

118. На "node3" выполните команды:

     ```bash
     docker node rm node2 # Удаляем node2 из кластера
     docker stack rm app  # Останавливаем приложение
     ```


Дальнейшая работа с кластером заключается в развёртывании приложений и выполняется точно также как мы это делали ранее в кластере с одним узлом.
