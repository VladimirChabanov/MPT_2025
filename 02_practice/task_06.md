# Практическое задание 6. Знакомство с Jenkins

**Цель:** Ознакомиться на практике со средствами CI/CD доступными в Jenkins.

## Установка

Скачать Jenkins можно с [официального сайта](https://www.jenkins.io/download/). Здесь доступно множество вариантов, под разные ОС, в том числе и Jenkins запакованный в Docker-контейнер. В этой работе мы будем устанавливать Jenkins с нуля на Ubuntu.

1. Создайте **новую** виртуальную машину с Ubuntu Server-ом в качестве имени пользователя выберите любое кроме `jenkins`. На этом сервере будет выполняться сборка и тестирование приложения.  
   Если у вас остался сервер с предыдущих работ, то не удаляйте его, т.к. он понадобится далее.

2. В настройках VirutalBox переключите Сеть виртуальной машины в режим "Сетевой мост", затем залогиньтесь и определите ip-адрес машины (в моем случае это: 192.168.1.40). С хостовой машины установите соединение с сервером по ssh с помощью любого, удобного для вас, ssh-клиента.  
   Таким образом будет удобнее выполнять последующие шаги, т.к. можно будет просто копипастить команды.

3. Перейдите на оф. сайт Jenkins в [раздел установки](https://www.jenkins.io/doc/book/installing/linux/#debianubuntu) и воспользуйтесь инструкцией для установки Java.

   > Если будут ошибки в процессе обновления индексов, попробуйте установить DNS: `sudo resolvectl dns enp0s3 8.8.8.8`.

4. Теперь установите Jenkins.  
   Для Jenkins можно выбрать один из вариантов: "Long Term Support release" или "Weekly release". Для этой работы разницы нет, но в "Long Term Support release" обычно меньше багов, т.к. релиз не самый последний и баги уже могли успеть исправить, а "Weekly release" хоть и более свежий, но может содержать больше багов.

5. Теперь нужно выполнить начальную конфигурацию Jenkins через web-интерфейс.  
   Т.к. машина с сервером на котором установлен Jenkins находится в одной сети с нашей хвостовой машиной, мы можем получить доступ к Jenkins через браузер на хостовой машине, просто указав в адресной строке браузера ip-адрес сервера и порт 8080 (стандартный порт для Jenkins). В моем случае это: 192.168.1.40:8080.
...
7. Теперь Jebkins предложит 2 варианта установки плагинов:
   ...
   Выберите второй вариант и откажитесь от всех плагинов (в самом верху кнопка: None). Плагины можно будет легко установить/удалить в любое время, поэтому мы не будем устанавливать их сейчас, чтобы не тратить время.

8. Создайте пользователя-администратора Jenkins. Логин и пароль нужно будет вводить при входе в web-интерфейс Jenkins, поэтому установите что-то простое.
...
9. По умолчанию Jankins определяет язык интерфейса по настройкам браузера, поэтому вместо английского вы можете увидеть русский текст. Иногда удобнее использовать английский интерфейс (проще гуглить). Если хотите можете оставить русский, но чтобы получить возможность переключить интерфейс на английский язык нужно установить плагин:
   ...
   - Выберите в меню справа иконку ⚙️ "Настроить Jenkins" ("Manage Jenkins"), а затем "Плагины" ("Plugins");
   ...
   - Вернитесь обратно в раздел "Настроить Jenkins" ("Manage Jenkins"), а затем перейдите в раздел "Внешний вид" ("Appearance");
   - В поле под названием "Язык по умолчанию" ("Default Language"). Выберите "English - en";
...
Сейчас мы создадим самый простой job (задание) с настройками по умолчанию. Job будет состоять только из одного шага в результате которого на сервере будут запущены несколько обычных shell-команд.

1. Чтобы создать новое задание (job) для Jenkins:

   - на главной странице (Dashboard), в меню слева нажмите кнопку "New Item";
   - введите имя job-a: "Simple";
...
3. В появившемся поле наберите:
   ...
   В первой строке, мы используем переменную окружения, которую устанавливает Jenkins. В данном случае в переменной `JOB_NAME` хранится имя которое мы дали job-у при создании. В таких переменных хранится много полезной информации, поэтому изучите их (ссылка на список переменных расположена здесь же, перед полем ввода команд).  
...
4. После того, как вы нажмёте кнопку "Save" job будет сохранён и вас перекинет в меню управления текущим job-ом.
   ...
   <p align="center"><img src="./task_06_img/job_dash.png"></p>
...
6. Щёлкнув по этому пункту вы попадёте раздел управления результатами сборки. Здесь нас интересует пункт меню "Console Output". Изучите свой вывод.
   ...
   <p align="center"><img src="./task_06_img/job_out.png"></p>
...
1. Создайте новый каталог **на любой** машине (хвостовой или виртуальной), на которой у вас есть утилита git;
...
3. В разделе "Triggers" перечислены источники событий, которые могут инициировать запуск job-a. Поставьте галочку напротив триггера "Poll SCM". Этот триггер будет опрашивать GitHub в соответствии с заданной периодичностью и в случае обнаружения изменений будет запускаться job;

4. В развернувшемся подразделе "Schedule" нужно указать периодичность опроса репозитория в формате cron. Для этого можно воспользоваться подсказкой или [онлайн генераторами](https://crontab.guru/). Введите в это поле строку: `H/2 * * * *`.

   > В обычном формате cron нет символа `H`, в строке выше вместо него должен был быть символ `*`. Чтобы периодически выполняемые задачи (в случае если их несколько) не запускались одновременно и не создавали пиковые нагрузки на систему в Jenkins добавлен символ `H`. При использовании `H`, Job-ы будут запускаться не все одновременно, а со случайным отклонением.

   Здесь мы просим автоматически проверять репозиторий каждую чётную минуту + случайное отклонение в пределах до следующей чётной минуты. Это случайное отклонение будет выбрано один раз для конкретного job-а и больше не будет изменяться;
...
6. Сохраните job, но НЕ запускайте его вручную. Подождите до ближайшей чётной минуты (+ 2-х минутный интервал) и job должен будет запуститься автоматически. Затем подождите ещё 3-5 минут. Т.к. репозиторий не изменялся, то и новых запусков в истории появляться НЕ должно;
...
9. Подождите несколько минут и вы должны увидеть, что запустился новый процесс сборки, а в рабочем каталоге появился файл `"other_file.png"`;

10. Плюс данного метода заключается в том, что машине с Jenkins не нужен белый IP, достаточно просто выхода в интернет. Минус данного метода в том, что мы вынуждены постоянно выполнять проверку репозитория, при этом нагружая свой сервер и сервера GitHub. Поэтому, чтобы не напрягать GitHub своими запросами зайдите в "Configure" и **выключите job**. Переключатель вверху страницы переведите из положения "Enabled" в "Disabled" и сохраните.
...
На данном этапе Jenkins ждёт уведомлений от GitHub, но GitHub об этом не подозревает. Т.е. теперь мы должны настроить GitHub таким образом, чтобы он отправлял сообщения нашему Jenkins-у;

8. Чтобы GitHub знал кого ему нужно уведомить о событиях, ему нужно предоставить Webhook (т.е. ссылку), но с этим есть определённая проблема (если у вас не белый IP). Виртуальная машина находится за NAT (ваш роутер и маршрутизаторы провайдер выполняют трансляцию адресов и портов), и следовательно, снаружи (из интернета) к ней доступа нет.  
   Чтобы решить эту проблему воспользуемся туннелем `ngrok`:

   - Откройте раздел Download на [оф. сайте](https://ngrok.com/download);

   - Скопируйте команду из "Install ngrok via Apt" (в разделе Linux), чтобы установить ngrok через `apt`;

   - Вставьте и выполните её в терминале на виртуальной машине;
   ...
   - В результате ngrok сгенерирует ссылку которая будет выполнять роль web-хука для GitHub. Т.е. все HTTP запросы отправленные на этот URL попадут на наш сервер на порт 8080.  
...
13. Откройте Jenkins (наш текущий job) и теперь вы должны увидеть, что сборка была выполнена;

14. **Внимание:** выключите этот job и убедитесь, что job по таймеру тоже отключён иначе могут быть ошибки во время выполнения следующих заданий.

Минус данного метода в том, что мы зависим от ngrok и в случае перезапуска сервера или самого ngrok-a, будет сгенерирована новая ссылка, а значит придётся вносить изменения и на GitHub. Но если у вашего сервера есть белый IP или вы настроили туннелирование через VPS, то данный метод очень хороший.
...
После установки добавьте пользователя jenkins (можно и основного тоже) в группу docker, чтобы он мог запускать команды без `sudo` и перезапустите службу jenkins.service:
...
3. В корне локального репозитория создайте каталог `"docker"` и в этом каталоге создайте `"Dockerfile"` со следующим содержимым:
...
4. Закоммитьте изменения **и отправьте** их на GitHub;

5. Перейдите в Jenkins и создайте новый job типа "Freestyle project";

6. В разделе "Source Code Management" укажите те же настройки, что и ранее;

7. Перейдите в раздел "Build Steps" и создайте step типа "Execute shell" содержащий:
...
8. При помощи кнопки "+ Add build step" добавьте ещё один шаг типа "Execute shell" содержащий:

   ```bash
   docker run --rm -d --name=qrcodegen -p 5000:5000 qrcode
   sleep 5
   ```
...
11. Сохраните и запустите job вручную. Оцените консольный вывод и если ошибок нет, то переходите к следующему шагу.  
...
12. Теперь нужно настроить отправку контейнера на DockerHub. Но для этого придётся войти в свою учётную запись.  
    Чтобы не прописывать логин и пароль в открытом виде добавим их в специальное хранилище учётных данных (Credentials).

    - В разделе "Environment" (выше "Build Steps") поставьте галочку напротив пункта "Use secret text(s) or file(s)".
    ...
    - В развернувшемся подразделе нажмите на кнопку "Add" и выберите "Username and password (separated)".  А затем заполните поля "Username Variable" и "Password Variable" значениями "DOCHUB_USERNAME" и "DOCHUB_TOKEN" соответственно.  Под этими именами будут доступны наши учётные данные от DockerHub на каждом step-е job-a.
      ...
      <p align="center"><img src="./task_06_img/secret.png"></p>

    - Нажмите кнопку "+ Add" (рядом с полем ввода "Credentials") и в развернувшемся списке выберите единственный доступный вариант: "Global";

    - В появившемся окне выберите "Username with password".

      <p align="center"><img src="./task_06_img/add_credentials.png"></p>

    - В открывшемся окне введите логин и пароли от DockerHub, а также поле ID.

      <p align="center"><img src="./task_06_img/log_pas.png"></p>

    - После того, как вы нажмёте "Create" окно закроется, а в поле "Credentials" будут выбраны только что введённые значения. В дальнейшем (в других job-ax) повторный ввод логина и пароля не потребуется, они сразу же будет доступны в списке.

13. Перейдите в раздел "Build Steps" и добавьте после последнего шага новый шаг типа "Execute shell" содержащий:

    ```bash
    docker login -u $DOCHUB_USERNAME -p $DOCHUB_TOKEN
    ```

14. Добавьте ещё один шаг типа "Execute shell" содержащий:
...
15. Следующим шагом выполняем push на DockerHub:
...
17. Откройте [DockerHub](https://hub.docker.com/) залогиньтесь под своим аккаунтом (может понадобиться VPN) и создайте репозиторий "qrcode".

18. Запустите job в ручном режиме.  
    В результате его выполнения на сайте [DockerHub](https://hub.docker.com/) вы должны увидеть новый образ в репозитории "qrcode";

19. Выполним окончательную настройку данного job-a, а именно настроим триггер, который будет следить за изменениями в репозитории на GitHub;

    В разделе "Triggers"  выберите и настройте "Poll SCM" или "GitHub hook trigger for GITScm polling" по своему усмотрению;

20. Для проверки работоспособности job-a. Замените содержимое файл `"main.py"` в локальном репозитории на следующее:
...
21. Выполните коммит, пуш на GitHub и убедитесь, что через некоторое время на [DockerHub](https://hub.docker.com/) появилась свежая версия приложения.

22. Выключите этот job.
...
2. Теперь нужно добавить машины на которые мы собираемся выполнять "деплой" (в нашем случае будет только одна):
   ...
   - Перейдите в web-интерфейс Jenkins и в меню "Настроить Jenkins" ("Manage Jenkins") выберите "Система" ("System");

   - Промотайте страницу почти до самого конца до радела "Publish over SSH" и здесь найдите подраздел "SSH Servers" и нажмите кнопку "Add";
   ...
   - В этом же блоке, чуть ниже разверните подраздел "Advanced" и в поле "Port" введите тот порт 6543;
     ...
     <p align="center"><img src="./task_06_img/ssh_servers.png"></p>
...
8. Теперь нужно настроить процесс пересылки файлов на целевую машину:
   ...
   - Введите в поле "Username" имя пользователя "proger", а в поле "Passphrase / Password" введите пароль. Убедитесь, что всё правильно, при помощи кнопки "Test Configuration".
     ...
     <p align="center"><img src="./task_06_img/serv_acsses.png"></p>
   ...
   - Ниже в разделе "Transfers" указывается что и куда будет пересылаться. Мы будем пересылать только файлы `"main.py"` и `"requirements.txt"`, которые попадут в рабочую директорию Jenkins из репозитория в папку `"app"` расположенную в домашней директорию пользователя proger.  
     ...
     <p align="center"><img src="./task_06_img/transfer.png"></p>

9. Сохраните настройки job-а;

10. Внесите любые изменения в репозиторий (не забудьте отправить на GitHub), чтобы активировать триггер;

11. Проверьте, что job отработал и файлы с GitHub действительно скопированы в каталог `"app"` расположенный в домашней директории пользователя proger второй виртуальной машины.

12. Выключите этот job.
...
В Jenkins есть возможность описывать задания в виде файла-скрипта, который называется - Pipeline.
...
4. Нажмите кнопку "Save", а затем "Build Now".  
   ...
   <p align="center"><img src="./task_06_img/first_pipeline.png"></p>
...
2. Создайте новую папку `"cpp_hello_world"` и в ней инициализируйте git-репозиторий (этот и последующие шаги можно выполнять либо на виртуальной либо на хостовой системе);
...
12. Сохраните и запустите job.  
    ...
    <p align="center"><img src="./task_06_img/git_pipeline.png"></p>
...
11. В таком виде отправка артефактов на сервер не произойдёт, т.к. мы не указали ни пароль ни ключ для подключения к серверу, поэтому прежде чем запускать job перейдите в раздел "Manage Jenkins" -> "System" и проскрольте до "Publish over SSH".  
...
12. Сохраните изменения и запустите job. Сервер, разумеется, должен быть запущен;
    ...
    > Поменяйте значение параметра `FILE_NAME` на другое иначе получите ошибку на этапе копирования файла на сервер, т.к. там уже присутствует каталог с именем `"app"`.

13. Если всё завершилось успешно, то проверьте, что в домашнем каталоге пользователя "proger" присутствует файл с указанным вами именем;  
    К сожалению вы не сможете его запустить из под пользователя "proger", т.к. сейчас у файла ответствует право `x` для всех (`other`), но если добавить это право, то файл выведет сообщение "Hello World";

    Почему же с python не было проблем и скрипт запускался после его копирования на сервер из под пользователя "proger"?  
